...
var thirdSectionText =
  "Cookies are small text files that are used by websites to improve the user experience. The law states that we can store cookies on your device if they are strictly necessary for the operation of this website. For all other types of cookies, we need your permission. This means that cookies classified as necessary are managed according to Article 6(1)(f) of the GDPR. All other cookies, such as those in the Preferences and Marketing categories, are managed according to Article 6(1)(a) of the GDPR. This website uses different types of cookies. Some cookies are set by third-party services that appear on our pages.";
...
var websiteLogo =
  "https://cdn.webshopapp.com/shops/141542/files/373013418/shakaloha-black-svg.svg";

var logoWidth = "50px";

var darkModeEnable = true;

var darkModeDefault = localStorage.getItem("viewMode")
...
window.addEventListener("load", function () {
  ...
  async function fetchCookies() {
    const response = await fetch(
      "https://cdn.jsdelivr.net/gh/consentbyalifmahmud-com/Consent-Banners@7f0cf88062efb8fdd993eff5167c80f73dc72444/metricsrealm.com/cookie.json"
    );
  ...
  websiteCookie.forEach((cookie) => {
    ...
    if (dataKey) {
      let isManualCookiePresent = manualCookie.some(
        (manual) => dataKey === manual.data_key
      );

      if (!isManualCookiePresent) {
        let justifiedValue =
          justifyCookies.find((justify) =>
            dataKey.includes(justify.dataKey_name)
          )?.dataKey_value || dataKey;
  ...
  function categoriesCookies(sendCookies) {
    ...
    sendCookies.forEach(function (sendCookies, index) {
      if (
        sendCookies.category == "Functional" ||
        sendCookies.category == "Necessary"
      ) {
        if (
        ...
        tempCategories.Necessary.allProviders[sendCookies.data_controller].push(
          sendCookies
        );
        ...
        tempCategories.Preferences.allProviders[
          sendCookies.data_controller
        ].push(sendCookies);
        tempCategories.Preferences.totalCookies += 1;
      } else if (
        sendCookies.category == "Analytics" ||
        sendCookies.category == "Statistics"
      ) {
        if (
        ...
        tempCategories.Statistics.allProviders[
          sendCookies.data_controller
        ].push(sendCookies);
        ...
        tempCategories.Marketing.allProviders[sendCookies.data_controller].push(
          sendCookies
        );
        ...
        if (
          ...
          tempCategories.Unclassified.allProviders[
            sendCookies.data_controller
          ] = [];
        }
        tempCategories.Unclassified.allProviders[
                ...
                <div class="cookieCategoryWrapper">
                    <div class="cookieCategoryTitle">
                        <div class="cookieCategoryTitleLeft">
                            ...
                            <span class="cookieHeadline categoryTitle">${
                              categoryText[index]
                            }</span>
                    ...
                    <div class="cookieCategoryDescriptions cookieDescription">${
                      category.description
                    }</div>
                    <div class="allCookiesOuter ${
                      totalCookies === 0 ? "" : "displayNone"
                    }">`;
            ...
            categoryHTML += `
                            <div class="mainCookiesDetails displayNone">
                                <div class="mainCookieWrapper">
                                    <span class="mainCookieName cookieHeadline">${
                                      cookie.data_key
                                    }</span>
                                    <span class="mainCookieDescription">${
                                      cookie.description
                                    }</span>
                                    <div class="bannerhr"></div>
                                    <span class="cookieHeadline">${
                                      cookieInfoText[0]
                                    }: <span class="cookieValue">${
              cookie.retention_period
            }</span></span>
                                    <span class="cookieHeadline">${
                                      cookieInfoText[1]
                                    }: <span class="cookieValue">${
              cookie.domain ? cookie.domain : window.location.host
            }</span></span>
                                    <span class="cookieHeadline">${
                                      cookieInfoText[2]
                                    }: <span class="cookieValue">${
  ...
  function restCodes() {
    ...
    var cookieCategoriesTitle = document.querySelectorAll(
      ".cookieCategoryTitleLeft"
    );
    var allCookieDetails = document.querySelectorAll(".allCookiesOuter");

    var allCookieWrapperTitle = document.querySelectorAll(
    ...
    function darkMode() {
      var lightModeRoots = {
        cookieHeadlineFontSize: rootStyle.getPropertyValue(
          "--cookieHeadlineFontSize"
        ),
        cookieHeadlineFontWeight: rootStyle.getPropertyValue(
          "--cookieHeadlineFontWeight"
        ),

        cookieDescriptionFontSize: rootStyle.getPropertyValue(
          "--cookieDescriptionFontSize"
        ),
        cookieDescriptionFontWeight: rootStyle.getPropertyValue(
          "--cookieDescriptionFontWeight"
        ),
        cookieDescriptionLineHeight: rootStyle.getPropertyValue(
          "--cookieDescriptionLineHeight"
        ),

        cookieTotalFontSize: rootStyle.getPropertyValue(
          "--cookieTotalFontSize"
        ),

        bannerFont: rootStyle.getPropertyValue("--bannerFont"),

        cookieHeadlineColor: rootStyle.getPropertyValue(
          "--cookieHeadlineColor"
        ),
        cookieDescriptionColor: rootStyle.getPropertyValue(
        ...
        activeButtonHoveColor: rootStyle.getPropertyValue(
          "--activeButtonHoveColor"
        ),
      };
      ...
      function dark() {
        root.style.setProperty(
          "--cookieHeadlineColor",
          darkModeRoots.cookieHeadlineColor
        );
        root.style.setProperty(
          "--cookieDescriptionColor",
          darkModeRoots.cookieDescriptionColor
        );

        root.style.setProperty(
          "--bannerAccentColor",
          darkModeRoots.bannerAccentColor
        );
        root.style.setProperty(
          "--bannerColorWhite",
          darkModeRoots.bannerColorWhite
        );
        root.style.setProperty(
          "--mainCookieBgColor",
          darkModeRoots.mainCookieBgColor
        );
        root.style.setProperty(
          "--InsideBorderColor",
          darkModeRoots.InsideBorderColor
        );

        root.style.setProperty(
          "--cookieValueWeight",
          darkModeRoots.cookieValueWeight
        );
        root.style.setProperty(
          "--normalButtonColor",
          darkModeRoots.normalButtonColor
        );
        root.style.setProperty(
          "--activeButtonHoveColor",
          darkModeRoots.activeButtonHoveColor
        );
      ...
      function light() {
        ...
        root.style.setProperty(
          "--cookieHeadlineColor",
          lightModeRoots.cookieHeadlineColor
        );
        root.style.setProperty(
          "--cookieDescriptionColor",
          lightModeRoots.cookieDescriptionColor
        );

        root.style.setProperty(
          "--bannerAccentColor",
          lightModeRoots.bannerAccentColor
        );
        root.style.setProperty(
          "--bannerColorWhite",
          lightModeRoots.bannerColorWhite
        );
        root.style.setProperty(
          "--mainCookieBgColor",
          lightModeRoots.mainCookieBgColor
        );
        root.style.setProperty(
          "--InsideBorderColor",
          lightModeRoots.InsideBorderColor
        );
        ...
        root.style.setProperty(
          "--normalButtonColor",
          lightModeRoots.normalButtonColor
        );
        root.style.setProperty(
          "--activeButtonHoveColor",
          lightModeRoots.activeButtonHoveColor
        );
    ...
    function showHide(mainBanner, miniBanner) {
      ...
      var bannerContentActive = document.querySelectorAll(
        ".bannerContentActive"
      );
    ...
    allTopSection.forEach((element, index) => {
      element.onclick = () => {
        if (index == 1) {
          ...
          setTimeout(function () {
            Object.keys(getChoice ? getChoice : {}).forEach(function (
              choice,
              index
            ) {
              var tempVar;
    ...
    allCookieWrapperTitle.forEach((element, index) => {
      element.onclick = () => {
        allCookieWrapper[index]
          .querySelectorAll(".mainCookiesDetails")
          .forEach(function (mainCookie) {
            if (mainCookie.classList.contains("displayNone")) {
              mainCookie.classList.remove("displayNone");
              mainCookie.classList.add("displayFlex");
              allCookieWrapperTitle[index]
                .querySelector(".allCookieTitleRight > svg")
                .classList.add("rotate");
            } else {
        ...
        if (element.id == "consentCustom") {
          ...
          if (onClickCustom == "normal") {
            ...
            Object.keys(checkedCategories).forEach((inputCategory, index) => {
              var convert = bannerCheckBox[index].checked
                ? "granted"
                : "denied";
  ...
  function secondBanner() {
    ...
    if (getSecondBanner) {
      ...
      function showSecondBanner() {
        ...
        var cookieBannerWrapper = document.querySelector(
          ".cookieBannerWrapper"
        );
